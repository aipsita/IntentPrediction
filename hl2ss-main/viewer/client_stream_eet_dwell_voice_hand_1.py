...
HEAD_VELOCITY_THRESHOLD = 0.05  # units/s, adjust as needed
...
def data_collection():
    ...
    try:
        ...
        timer = schedule_function1()  # This starts the first timer
        while enable:
...
def send_signal_to_unity_voice(message):
    buffer = hl2ss_rus.command_buffer()
    buffer.connect_success_voice(message)
    ipc.push(buffer)
    response = ipc.pull(buffer)
...
def audio_recording():
    ...
    while enable:
        ...
        if is_speaking(audio_data):
            ...
            send_signal_to_unity_voice('UserSpeaking')
        #else:
            #print("User is not speaking")
            #send_signal_to_unity_voice('NotSpeaking')
    ...
    while enable:
        ...
        if si.is_valid_hand_right():
            ...
            if len(data) >= 20:
                ...
                smoothed_centroids = uniform_filter(centroids, size=5)
                ...
                if significant_directions:
                    ...
                    if max_direction_count > len(significant_directions) / 2:
                        ...
                        if previous_dominant_direction == dominant_direction:
                            ...
                            if dominant_direction != 'no significant direction':
                                send_signal_to_unity("NoDirection")
                                #send_signal_to_unity_priority('NoDirection', priority=PRIORITY_HIGH)
                            if dominant_direction != 'no significant direction':
        ...
        if si.is_valid_head_pose():
            ...
            if len(data_head) >= FRAME_SIZE_HEAD:
                ...
                head_smoothed_positions = uniform_filter(head_positions, size=WINDOW_SIZE)
...
def determine_direction(vector):
    ...
    elif max_index == 2:  # Z component is largest
