...
while enable:
    ...
    if si.is_valid_hand_right():
        ...
        for joint_name, joint_index in joint_indices.items():
            if joint_name not in ['Wrist', 'IndexIntermediate', 'ThumbTip', 'ThumbDistal']:
        ...
        if len(data) >= 10:
            ...
            significant_direction_vectors_thumb = thumb_tips[1:][significant_indices] - thumb_distals[1:][significant_indices]
            ...
            directions_thumb = [determine_direction(vec) for vec in significant_direction_vectors_thumb]
            
            significant_directions = [direction for direction in directions if direction != 'no significant direction']
            significant_directions_thumb = [direction for direction in directions_thumb if direction != 'no significant direction']
            ...
            if significant_directions:
                ...
                elif significant_directions_thumb:
                    direction_counts_thumb = {direction: significant_directions_thumb.count(direction) for direction in set(significant_directions_thumb)}
                    max_direction_count_thumb = max(direction_counts_thumb.values())
                    
                    if max_direction_count_thumb > len(significant_directions_thumb) / 2:
                        dominant_direction = max(direction_counts_thumb, key=direction_counts_thumb.get)
                        if previous_dominant_direction == dominant_direction:
                            print(f"Dominant direction for thumb in last 10 frames: {dominant_direction}")
