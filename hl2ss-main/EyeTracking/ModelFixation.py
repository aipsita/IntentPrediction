...
def get_fixation(data):
    ...
    # Check the conditions for the entire dataset
    last_fixation_event = new_data['is_fixation_int'].iloc[-1]
    fixation_event_sum = new_data['is_fixation_int'].sum()
    saccade_event_sum = new_data['is_saccade_int'].sum()
    #print(f"Fixation: {fixation_event_sum}")
    #print(f"Saccade: {saccade_event_sum}")

    # Set prediction based on the conditions
    if fixation_event_sum > 20 and saccade_event_sum == 0 and last_fixation_event == 1:
        prediction = 1
    else:
        prediction = 0
